...
{
    cin.ignore();
    cout << "Enter an item name: "; getline(cin, items[counter]);
    cout << "Enter the cost of the item: "; cin >> costs[counter];
    cout << "Enter the number of items you are buying: "; cin >> quantity[counter];
    subtotal += computeItemTotal(quantity[counter], costs[counter]);
    counter++;
}
void calculateTotal(double  subtotal, double& total)
{
                  ...
                  string items[], double costs[], int quantity[])
{
    ...
    {
        cout << items[i] << " " << quantity[i] << " $" << costs[i];
        cout << endl;
    }
    cout << "===========================================" << endl;
